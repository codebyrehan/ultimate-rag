...
class User(Base):
    ...
    __table_args__ = ({"sqlite_autoincrement": False},)  # IDs are app-generated ULIDs
...
class Document(Base):
    ...
    filename: Mapped[str] = mapped_column(String(320), nullable=False)  # sanitized storage name
    ...
    indexing_status: Mapped[DocStatus] = mapped_column(
        Enum(DocStatus, name="doc_status"), default=DocStatus.UPLOADED, nullable=False
    )
    ...
    __table_args__ = (
        # per-tenant duplicate-name guard is not enforced; SHA-256 dedups globally
...
class Job(Base):
    ...
    kind: Mapped[str] = mapped_column(String(64), nullable=False)  # e.g. "ingest"
